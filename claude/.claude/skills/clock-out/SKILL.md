---
name: clock-out
description: End the workday. Write a session retro, finalize today's daily note with a summary and next-session handoff, then commit the journal once. Use when the user says "clock-out", "퇴근", "오늘 마무리", "정리하자", or is wrapping up for the day.
---
...
7. **하루 1커밋** — `cd ~/vaults/journals && git add -A && git commit -m "journal: <TODAY>"`.
