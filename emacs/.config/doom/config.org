...
(after! org
  ...
  ;; The scale handed to dvisvgm is image-size-adjust * :scale * (display-dpi / 140),
  ;; and org--get-display-dpi divides the pixel height by the compositor scale factor.
  ;; So the same config renders differently per machine: 70 dpi on the 4K desktop at
  ;; scale 2 (half size), higher on the laptop. Cancel the dpi term so previews come
  ;; out the same everywhere; 1.3 is the taste knob. Needs a frame, hence the hook.
  (defun js/org-set-latex-preview-scale ()
    (when (display-graphic-p)
      (plist-put org-format-latex-options
                 :scale (* 1.3 (/ 140.0 (org--get-display-dpi))))))
  ;; window-setup-hook covers a plain GUI start, the server hook covers emacsclient
  ;; frames; both run after the setq! below, which would otherwise reset :scale.
  (add-hook 'window-setup-hook #'js/org-set-latex-preview-scale)
  (add-hook 'server-after-make-frame-hook #'js/org-set-latex-preview-scale)
...
:END:
#+begin_src emacs-lisp
;; Emacs 30.2's treesit.c serialises the `:match' and `:pred' query predicates
;; as "#match"/"#pred", but libtree-sitter >= 0.25 (0.26.8 here) requires a
;; predicate name to end in ? or !.  python-ts-mode's keyword, builtin and type
;; font-lock queries therefore fail to compile, and inside an Org src block
;; `org-src-font-lock-fontify-block' reports each failure as a "Native code
;; fontification error" -- once per keystroke.  Fixed upstream by b01435306a3
;; (bug#79687); until this Emacs is rebuilt, fontify src blocks with the
;; non-tree-sitter major modes, which have no such queries.
(after! org-src
  (defadvice! +org-src-fontify-without-treesit-a (fn &rest args)
    :around #'org-src-font-lock-fontify-block
    (let ((major-mode-remap-defaults nil)
...
(use-package! ob-jupyter
  ...
  ;; Org 9.8's `org-element-drawer-interpreter' does (make-string (:pre-blank
  ;; drawer) ?\n), but emacs-jupyter builds its RESULTS drawer without that
  ;; property.  The (wrong-type-argument wholenump nil) it raises is swallowed
  ;; by the message publisher, so a block that errors in the kernel reports
  ;; only "Error in ZMQ subprocess filter" instead of the Python traceback.
  (defadvice! +jupyter-org-drawer-pre-blank-a (drawer)
    :filter-return #'jupyter-org-results-drawer
    (if (eq (org-element-type drawer) 'drawer)
        (org-element-put-property drawer :pre-blank 0)
      drawer))
  ;; company (idle 0.2s) runs `jupyter-org-completion-at-point' on nearly every
  ;; keystroke inside a jupyter src block, which sends a complete_request to the
  ;; kernel.  When a client's ZMQ channel is in a bad state that send reaches a
  ;; nil socket in the ioloop subprocess -- (wrong-type-argument user-ptrp nil)
  ;; -- and the echo area is spammed once per keystroke.  Completion is not
  ;; needed to run a block, so keep the kernel out of the typing path.
  (add-hook! 'jupyter-org-interaction-mode-hook
    (defun +jupyter-org-drop-kernel-capf-h ()
      (remove-hook 'completion-at-point-functions
                   'jupyter-org-completion-at-point t)))
  ;; `zmq--subprocess-filter' re-signals subprocess errors from inside a process
  ;; filter, which Emacs reports in the echo area.  Log them instead; errors
  ;; raised by the kernel itself arrive as Jupyter messages and still get
  ;; inserted into the Org buffer.
  (with-eval-after-load 'zmq
    (defadvice! +jupyter-quiet-zmq-subprocess-error-a (fn process output)
      :around #'zmq--subprocess-filter
      (condition-case err
          (funcall fn process output)
        (zmq-subprocess-error
         (let ((inhibit-message t))
           (message "ZMQ subprocess error (silenced): %S" err)))))))
#+end_src
...
(defun js/dired-open-external ()
  ...
  (let ((opener (cond ((executable-find "rifle")
                      ...
                      ((executable-find "open")
                       "open"))))
    (unless opener
      (user-error "No external opener found (rifle, xdg-open or open)"))
